...
1. [系統架構總覽](#1-系統架構總覽)
2. [Ansible 跳板機 — 初始化安裝](#2-ansible-跳板機--初始化安裝)
3. [API Bridge 啟動與設定](#3-api-bridge-啟動與設定)
4. [Web 介面使用說明](#4-web-介面使用說明)
   - [4.1 TiDB 頁（資料庫指令）](#41-tidb-頁資料庫指令)
   - [4.2 Redis 頁（Kuboard 連結）](#42-redis-頁kuboard-連結)
   - [4.3 部署更新頁（GitLab Image Tag）](#43-部署更新頁gitlab-image-tag)
5. [PROD 環境安全機制](#5-prod-環境安全機制)
6. [故障排除](#6-故障排除)
7. [安全建議](#7-安全建議)
...
> ⚠️ API Bridge 預設只監聽 `localhost:8765`，若 Web 介面與跳板機不在同一網路，需建立 SSH Tunnel 或設定 Nginx 反代（詳見 service 檔案內附設定）。
...
1. 填入 GitLab API URL（`https://git.trevi.cc`）
2. 填入 Project ID（到 GitLab 專案頁面 → Settings → General 查詢）
3. 填入 Personal Access Token（需要 `api` scope）
4. 選擇環境、類型（Game / PP）、專案
5. 點擊模塊 chip 選擇要更新的服務（支援多選）
6. 填入 Image Tag（格式：`YYYYMMDDHHmmSS-{shortSHA}-{seq}`）
7. 點擊「預覽變更」確認路徑正確
8. 點擊「提交 GitLab」（PROD 需額外輸入「確認部署」）
